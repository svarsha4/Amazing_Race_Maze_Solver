...
- **Queue & Breadth First Search:**
    - The First In First Out principle enables the ability to easily keep track of the next possible set of coordinates (row, col) to go through in the maze.
    - This is key for implementing the BFS algorithm, which is used in this function to find the shortest path through the maze. BFS is implemented here by exploring all the possible paths than can be taken by level.
